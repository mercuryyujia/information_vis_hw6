...
function AirportBubble(props){
    ...
    const renderBubbles = (cities) => {
        ...
        // Initialize positions for cities
        cities.forEach((d) => {
            d.x = width / 2; // Initial X position
            d.y = height / 2; // Initial Y position
        });
        ...
        // Render circles and text for cities
        return cities.map((d, idx) => {
            const isTopHub = idx >= cities.length - 5; // Top 5 hubs are the last 5 elements (sorted ascendingly)
            return (
                <g key={idx}>
                    <circle
                        cx={d.x}
